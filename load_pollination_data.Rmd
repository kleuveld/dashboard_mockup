...
Willem-Jan sent some more interesting data; let's see if we can put that into a map:


```{r eval=TRUE, include = TRUE, echo=TRUE, warning=TRUE, error=TRUE, message=FALSE}

production_volume <- read_csv(here("data/Production_volume.csv"))
...
mapdata2 <-
  production_volume_geo %>%
  filter(ACTS == "pdr") %>%
  filter(Scenario == "GDPEndoSSP2_noWaste") %>%
  filter(Year == 2017)


mypalette <- colorBin(
  palette = "YlOrBr", domain = mapdata2$Value,
...
leaflet(mapdata2) %>%
        ...
        addLegend(
          pal = mypalette, values = ~Value, opacity = 0.9,
          title = "Production change", position = "bottomleft"
        )


unique(production_volume_geo$ACTS)
```